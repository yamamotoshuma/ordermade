...
<script>
    ...
    // フォームの各要素を取得
    const McodeSelect = document.getElementById('Mcode');
    const ScodeSelect = document.getElementById('Scode');
    const disburYearInput = document.getElementById('disbur_year');
    const disburMonthSelect = document.getElementById('disbur_month');
    const disburAmountInput = document.getElementById('disbur_amount');

    var date = new Date();

    // フォームの各要素の値をローカルストレージから取得してセット
    McodeSelect.value = localStorage.getItem('Mcode') || '';
    ScodeSelect.value = localStorage.getItem('Scode') || '';
    disburYearInput.value = date.getFullYear();
    disburMonthSelect.value = date.getMonth()+1;
    disburAmountInput.value = localStorage.getItem('disbur_amount') || '';

    // フォームの各要素の値が変更されたら、ローカルストレージに保存
    McodeSelect.addEventListener('change', function() {
        localStorage.setItem('Mcode', McodeSelect.value);
    });

    ScodeSelect.addEventListener('change', function() {
        localStorage.setItem('Scode', ScodeSelect.value);
    });

    disburAmountInput.addEventListener('input', function() {
        localStorage.setItem('disbur_amount', disburAmountInput.value);
    });
});
</script>
</x-app-layout>
